...
class MqttHelper:
    ...
    def dev_unique_id(self, device_id: str, entity: str = "") -> str:
        return "_".join(
            [self.device_slug(device_id), re.sub(r"[^a-zA-Z0-9]+", "", entity)]
        )
    ...
    def device_slug(self, device_id: str) -> str:
        return "_".join(
            filter(None, [self.service_slug, re.sub(r"[^a-zA-Z0-9]+", "", device_id)])
        )
    ...
    def disc_t(self, component: str, item: str) -> str:
        ...
        return "/".join(
            ["homeassistant", component, self.service_slug + "_" + item, "config"]
        )
    ...
    def stat_t(self, device_id: str, category: str, *parts: str) -> str:
        ...
        return "/".join(
            [self.service_slug, self.device_slug(device_id), category, *map(str, parts)]
        )

    def avty_t(
        self, device_id: str, category: str = "availability", *parts: str
    ) -> str:
        if device_id == "service":
            return "/".join([self.service_slug, category, *map(str, parts)])
        return "/".join(
    ...
    def attr_t(self, device_id: str, category: str = "attributes", *parts: str) -> str:
        if device_id == "service":
            return "/".join([self.service_slug, category, *map(str, parts)])
        return "/".join(
            ["homeassistant", self.device_slug(device_id), category, *map(str, parts)]
        )
    ...
    def safe_publish(
        self, topic: str, payload: str | bool | int | dict | None, **kwargs: Any
    ) -> None:
        if not topic:
        ...
        try:
            ...
            self.logger.warning(
                f"MQTT publish failed for {topic} with {payload[:120] if isinstance(payload, str) else payload}: {e}"
            )
